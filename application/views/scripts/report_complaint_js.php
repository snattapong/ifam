...
<script>
console.log("report/complaint script");
...
var table=$('#<?=$method?>-table').DataTable({
	...
	buttons: [
		...
		{  text: 'RESET',
		   className: 'btn-primary table-btn',
			action: function(){ 
				table.ajax.url( '<?= site_url('command/getComplaints') ?>' ).load();
			}
		}
   ],
	ajax : "<?= site_url('command/getComplaints') ?>",
	rowId : 'id',
	columns : [ 
		{ title : "เลขที่", data : "documentNum" },	
		{ title : "วันที่", data : "recieveDate" },	
		{ title : "ประเภทร้องเรียน", data : "comptypeName" },	
		{ title : "เรื่อง", data : "title" },	
...
$('#filter-btn').click(function(){
   ...
   var toDate = $('#toDate').val();
	...
	table.ajax.url( '<?=site_url('command/getComplaintsBetween')?>/'+fromDate+'/'+toDate ).load();
});
