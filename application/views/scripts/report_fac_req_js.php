...
<script>
console.log("report/fac_req script");
...
var table=$('#<?=$method?>-table').DataTable({
	...
	buttons: [
		...
		{  text: 'RESET',
		   className: 'btn-primary table-btn',
			action: function(){ 
				//alert('กำลังจะล้าง ใจเย็นๆ');

				table.ajax.url( '<?= site_url('command/getRequestFactories') ?>' ).load();
			}
		}
   ],
	ajax : "<?= site_url('command/getRequestFactories') ?>",
	rowId : 'id',
	columns : [ 
...
$('#filter-btn').click(function(){
   ...
   var toDate = $('#toDate').val();
	...
	table.ajax.url( '<?=site_url('command/getCertFacBetween')?>/'+fromDate+'/'+toDate ).load();

	/*
	$.get('<?= site_url('command/getCertFacBetween')?>'
		,{ fromDate : fromDate , toDate : toDate }
		,function(data){
			console.log(data);
			var datatable = $('#<?=$method?>-table').dataTable().api();
			datatable.clear();
